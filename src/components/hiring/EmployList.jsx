...
class EmployList extends Component {
  ...
  render() {
    const columns = [{
      ...
      render: (text, record, index) => <span style={{color: record.key == 1 ? "black" : "red"}}>{record.status}</span> 
    }, {
      title: 'Id',
      dataIndex: 'id',
      key: 'id',  
      ...
      render: (text, record, index) => (<Link to={"/hiring/employdetail/" + record.id} >{record.id}</Link>),   
    }, {
